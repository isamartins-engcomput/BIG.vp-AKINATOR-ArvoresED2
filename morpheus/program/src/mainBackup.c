...
void processa_linha(struct No **raiz, char *linha)
{
    ...
    if (atual != NULL && atual->eh_personagem == 'S')
...
void preenche_folhas(struct No **raiz)
{
    ...
    FILE *temp;
    filmes_e_series = fopen("../../baseDeDados/filmes_e_series64.txt", "r");
    int quantidade = 0;
    {
        char linha[100];
...
void Akinator(struct No **raiz, char perguntas[][51], int pergunta)
{
    if ((*raiz)->eh_personagem == 'N')
    {
        ...
        if (resposta == 's' || resposta == 'S')
            Akinator(&(*raiz)->sim, perguntas, pergunta + 1);
        else
            Akinator(&(*raiz)->nao, perguntas, pergunta + 1);
    }
    else
    {
